...
{
	__asm__ __volatile__(" mrs   x3, CNTKCTL_EL1         \n\t"
			     " str   x3, [%0, #0]           \n\t"
			     " mrs   x2, CNTP_CTL_EL0        \n\t"
...
{
	__asm__ __volatile__(" ldr   x3, [%0, #0]       \n\t"
			     " msr   CNTKCTL_EL1, x3         \n\t"
			     " ldp   x2, x3, [%0, #8]       \n\t"
			     " msr   CNTP_CTL_EL0, x2        \n\t"
...
_back:
	...
	CPU_DORMANT_INFO
	    ("dormant(flags:%x) (ret:%x) (core:%d/%d) cnt:%d, rst:%d, abt:%d, brk:%d\n",
...
int mt_cpu_dormant_init(void)
{
	...
	sleep_aee_rec_cpu_dormant =
	    virt_to_phys((void *)sleep_aee_rec_cpu_dormant_va);
