...
{
	u32 pcm_ptr, vcore_ptr, sodi_ptr;

	pcm_ptr = spm_read(SPM_PCM_IM_PTR);
	vcore_ptr = base_va_to_pa(__spm_vcore_dvfs.pcmdesc->base);
	sodi_ptr = base_va_to_pa(__spm_sodi.pcmdesc->base);
...
int spm_set_vcore_dvs_voltage(unsigned int opp)
{
	...
	if (is_fw_not_existed() || (opp == OPP_0 && is_fw_not_support_uhpm()))
		__go_to_vcore_dvfs(SPM_VCORE_DVFS_EN);
