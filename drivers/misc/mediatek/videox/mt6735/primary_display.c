...
//	mva = pgc->dc_buf[pgc->dc_buf_id];
    ...
    /*** this condition is for multi-pass but now it's never used. */
    // if(_is_decouple_mode(pgc->session_mode) && !_is_mirror_mode(pgc->session_mode))
    /*** */
	if(0) 
	{
        ...
        src_format = decouple_wdma_config.outputFormat;
        pixel_byte = DP_COLOR_BITS_PER_PIXEL(src_format)/8;
	}
	else
	{
        ...
        src_format = pconfig->wdma_config.outputFormat;
        pixel_byte = DP_COLOR_BITS_PER_PIXEL(src_format)/8;

	}
	buffer_size = h_yres*pitch;

    /* width*bpp is supposed to be equaled to pitch (or maybe smaller) */
	ASSERT( (pitch/pixel_byte)>=w_xres);
...
//	dpmgr_get_input_address(pgc->dpmgr_handle,&mva);
    ...
    /* Convert decouple buffer to ABGR */
	{
		unsigned int j = 0;
		unsigned char tem_va[8]; // maximum bytes in a pixel should be 4, for safety, give it 8 bytes.
		unsigned long src_va = va; // get source pointer

        /* for every pixels */
		for(i=0;i<h_yres;i++){
			for(j=0;j<w_xres;j++){

                unsigned long *pByte = (pbuf+(i*w_xres+j)*4); // source byte

                /* copy a pixel */
				memcpy((void*)tem_va, (void*)src_va, pixel_byte);
                
                switch(src_format){
                case DP_COLOR_BGR888:                   
                    *pByte = 0xFF<<24 | tem_va[2]<<16 | tem_va[1]<<8 | tem_va[0];
                    break;
                    
                case DP_COLOR_RGB888:
                    *pByte = 0xFF<<24 | tem_va[0]<<16 | tem_va[1]<<8 | tem_va[2];
                    break;
                    
                case DP_COLOR_BGRA8888:
                    *pByte = tem_va[3]<<24 | tem_va[0]<<16 | tem_va[1]<<8 | tem_va[2];
                    break;
                ...
                case DP_COLOR_ABGR8888:
                    memcpy((void*)pByte, (void*)tem_va, 4); // copy directly.
                    break;
                    
                default:
                    /* should be resolved if format not supported */                    
                    DISPERR("not support input buffer format, the input buffer format should be supported. \n");
                    ASSERT(0); 
                }
                                
				src_va+=pixel_byte; // shift source
			}
		}
	}
...
int primary_display_capture_framebuffer_ovl(unsigned long pbuf, unsigned int format)
{
 	...
 	if(_is_decouple_mode(pgc->session_mode) || _is_mirror_mode(pgc->session_mode))
    {
        ...
        /*** 2015.Feb.26 by Rynn.Wu */
        /*** Ignore filling black for decouple. But for security path, this may be necessary */   
        if( 0 /* TODO: determine if security path */ )
        {
            memset((void*)pbuf, 0, buffer_size);
