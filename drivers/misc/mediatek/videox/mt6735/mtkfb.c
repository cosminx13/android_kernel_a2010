...
static int mtkfb_pan_display_impl(struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	//except ASSERT_LAYER is enable
	if(is_DAL_Enabled()){
		session_input.config[primary_display_get_option("ASSERT_LAYER")].layer_id = 0;
...
static int mtkfb_set_par(struct fb_info *fbi)
{
   ...
   // DISPFUNC();
    ...
    /*
    The fbdev->fb_info has been changed as an invalid address in recovery mode.
    Map the fbdev->fb_info to fbi again to avoid memory page fault.
    This workaround should be removed after clariying invalid address issue.
    */
    fbdev->fb_info = fbi;
    /**/
