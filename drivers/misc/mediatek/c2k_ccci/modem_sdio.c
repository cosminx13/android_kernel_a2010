...
#define SDIO_MD_ID				(2)
#define CCMNI_AP_LOOPBACK_CH	(10)
...
struct ccmni_ccci_ops sdio_ccmni_ops = {
	...
	.irat_md_id			= MD_SYS1,
#else
	.md_ability			= 0, //MODEM_CAP_TXBUSY_STOP,
	.irat_md_id			= -1,
#endif
	.napi_poll_weigh	= 0,
...
static int check_port(struct sdio_modem_port *port)
{
	...
	if (!port){
		...
		if (modem->status == MD_OFF){
			LOGPRT(LOG_ERR, "%s %d: modem is off now.\n", __func__, __LINE__);
			return -ENODEV;
		}
	}
...
static void modem_heart_beat_poll_work(struct work_struct *work)
{
	...
	LOGPRT(LOG_ERR,  "start heart beat timer...\n");
	...
	ret = contruct_ctrl_chan_msg(ctrl_port, HEART_BEAT_MSG_ID, 0, 0);
	if (ret < 0){
...
static void c2k_force_assert_timer(unsigned long data)
{
	...
	snprintf(buff, AED_STR_LEN, "MD3 exception: %s", ex_info);
...
static int ctrl_msg_analyze(struct sdio_modem *modem)
{
	...
	switch(msg_id)
	{
		...
		case HEART_BEAT_MSG_ID:
			LOGPRT(LOG_INFO, "heart beat msg received\n");
...
static int modem_exception_handler(struct sdio_modem *modem)
{
	...
	if (excp_msg->ccci_head.data1 == C2K_MD_EX){
		...
		snprintf(buff, AED_STR_LEN, "md3: %s", ex_info);
	#if defined (CONFIG_MTK_AEE_FEATURE)
	if(debug_info->type == CC_MD1_EXCEPTION && debug_info->fatal_error.err_code1 == MD_EX_LTE_FATAL_ERROR){
		...
		aed_md_exception(NULL, 0, NULL, 0, buff);
	}
	#endif
	}
	return 0;
...
*/	
    ...
    if (pure_int & SDIO_CHISR_RX_RDY) {
		...
		if((modem->msg->head.chanInfo  == SDIO_AT_CHANNEL_NUM) || (modem->msg->head.chanInfo == EXCP_CTRL_CH_ID)) {//
...
retry_get_buf_in_room:
#if ENABLE_CHAR_DEV
						spin_lock(&port->sdio_buf_in_lock);
#else
						mutex_lock(&port->sdio_buf_in_mutex);
#endif
						...
						if(port->sdio_buf_in_size > SDIO_BUF_IN_MAX_SIZE){
							if (modem->status == MD_EXCEPTION || modem->status == MD_EXCEPTION_ONGOING){
								port->sdio_buf_in_size -= (modem->data_length - payload_offset);
#if ENABLE_CHAR_DEV
								spin_unlock(&port->sdio_buf_in_lock);
#else
								mutex_unlock(&port->sdio_buf_in_mutex);
#endif
								msleep(10);
		...
		void *buf, size_t len)
{
	...
	if((ch_id == DATA_CH_ID) || (ch_id == SDIO_AT_CHANNEL_NUM)) {//(index == SDIO_AT_CHANNEL_NUM) || 
...
#if ENABLE_CHAR_DEV
	spin_lock_init(&port->sdio_buf_in_lock);
#else
...
int modem_buffer_push(int port_num, void *buf, int count)
{
	...
	if(modem->status != MD_OFF){
		...
		if (count > (ONE_PACKET_MAX_SIZE - chars_in_fifo)){
			printk(KERN_DEBUG "[C2K]too many data pending...");
			return -ENOMEM;
		}
		ret = kfifo_in_locked(&port->transmit_fifo, buf, count, &port->write_lock);
		queue_work(port->write_q, &port->write_work);
...
get_one:
	...
	ret = copy_to_user(buf+read_len, packet->buffer+packet->offset, read_curr);
	...
	if(full_pkt_done) {
		...
		if(!list_empty(&port->sdio_buf_in_list) && user_available) {
			LOGPRT(LOG_DEBUG, "read on port%d more, %d/%zu\n", port->index, read_len, count);
			spin_unlock(&port->sdio_buf_in_lock);
			goto get_one;
		}
	}
...
exit:
	LOGPRT(LOG_DEBUG, "read on port%d done, %d/%zu\n", port->index, read_len, count);
...
static ssize_t dev_char_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
	...
	LOGPRT(LOG_DEBUG,  "write on port%d for %zu\n", port->index, count);
	return ret;
