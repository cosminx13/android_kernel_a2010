...
u8 is_reseting = 0;
#if GTP_RATE_BOOST
static void rb_add_record(u8 *coor_data, int touch_num);
#endif
...
s32 gt1x_read_version(struct gt1x_version_info * ver_info)
{
	...
	GTP_INFO("IC VERSION: GT%s_%04X(Patch)_%04X(Mask)_%02X(SensorID)", product_id, patch_id >> 8, mask_id >> 8, sensor_id);
...
s32 gt1x_touch_event_handler(u8 * data, struct input_dev * dev, struct input_dev * pen_dev)
{
	...
	s32 ret = -1;
#if !GTP_RATE_BOOST
	s32 id, input_x, input_y, input_w;
#endif
	GTP_DEBUG_FUNC();
...
#if GTP_RATE_BOOST
		coor_data = &touch_data[1];
		rb_add_record(coor_data, touch_num);
#else
		u8 report_num = 0;
		coor_data = &touch_data[1];
		id = coor_data[0] & 0x0F;
...
#if GTP_RATE_BOOST
#define RB_REPORT_RATE   8000000	//8ms == 125Hz report rate
...
struct rate_boost_t {
	struct coordinate buffer0;
	struct coordinate buffer1;
	spinlock_t lock;
	struct input_dev *input_dev;
	struct hrtimer xtimer;
	int record_empty;
};

static struct rate_boost_t rate_boost; 	//record id coordinates 
static struct coordinate *coor_new;
static struct coordinate *coor_old;

static inline void rb_report_start(void)
{
	int i;
	
	//spin_lock(&rate_boost.lock);
	for (i = 0; i < GTP_MAX_TOUCH; i++)
		if (coor_new->x[i] != -1)
			gt1x_touch_down(coor_new->x[i], coor_new->y[i], coor_new->w[i], i);	/* first point P0*/
	//spin_unlock(&rate_boost.lock);
	input_sync(rate_boost.input_dev);
	if (!hrtimer_active(&rate_boost.xtimer))
		hrtimer_start(&rate_boost.xtimer, ktime_set(0, RB_REPORT_RATE), HRTIMER_MODE_REL);
	rate_boost.record_empty = 0;
	//GTP_DEBUG("Rate boost timer started.");
}

/** 
 * rb_add_record - add coordinate data into buffer  
 *
 * @coor_data: coordinate data buffer. If touch_num
 *			==0 ,assign Null to it.
 * @touch_num: the touch number in the buffer,		
 */
static void rb_add_record(u8 *coor_data, int touch_num) 
{
	struct coordinate *coor_tmp;
	s32 input_x, input_y, input_w;
	int i, id, report_num = 0;
	unsigned long flags;

	//spin_lock_bh(&rate_boost.lock);
	spin_lock_irqsave(&rate_boost.lock, flags);
	coor_tmp = coor_new;
	coor_new = coor_old;
	coor_old = coor_tmp;
	coor_tmp = coor_new;
	...
	if(likely(touch_num)) {
		...
		for (i = 0; i < GTP_MAX_TOUCH; i++) {
			if (i == id) {
				...
				coor_tmp->x[i] = input_x;
				coor_tmp->y[i] = input_y;
				coor_tmp->w[i] = input_w;
				if (report_num++ < touch_num) {
					coor_data += 8;
					id = coor_data[0] & 0x0F;
				}
		...
		do_gettimeofday(&coor_tmp->t);		/* get time value */
	} else {			
		for (i = 0; i < GTP_MAX_TOUCH; i++) 
			coor_tmp->x[i] = -1;			/* x[i] = -1: no coor data */
	}
	//spin_unlock_bh(&rate_boost.lock);
	
	if (rate_boost.record_empty) {
		rb_report_start();	/* first touch, start timer */
	}
	spin_unlock_irqrestore(&rate_boost.lock, flags);
	
}

/** 
 * rate_boost_func - hrtimer callback function  
 */
static enum hrtimer_restart rate_boost_func(struct hrtimer *timer)
{
	struct rate_boost_t *rate_boost = container_of(timer, struct rate_boost_t, xtimer);
	struct timeval time;
	s32 cnv_x, cnv_y, cnv_w;	
	s32 delta_x, delta_y, delta_t, vt;
	int i, touch_num = GTP_MAX_TOUCH;

	do_gettimeofday(&time);
	spin_lock(&rate_boost->lock);
	vt = (time.tv_sec - coor_new->t.tv_sec)*1000000 +
		 time.tv_usec - coor_new->t.tv_usec;		/* us */
	
	for (i = 0; i < GTP_MAX_TOUCH; i++) {
		if (likely(coor_new->x[i] != -1)) {
			if (likely(coor_old->x[i] != -1)) {
				delta_x = coor_new->x[i] - coor_old->x[i];
				delta_y = coor_new->y[i] - coor_old->y[i];
				delta_t = (coor_new->t.tv_sec - coor_old->t.tv_sec) * 1000000
						  +coor_new->t.tv_usec - coor_old->t.tv_usec;		
				
				cnv_x = coor_new->x[i] + (delta_x) * vt / delta_t;
				cnv_y = coor_new->y[i] + (delta_y) * vt / delta_t;
				cnv_w = coor_new->w[i];	/* Undecided Algorithms */
			} else {
				cnv_x = coor_new->x[i];
				cnv_y = coor_new->y[i];
				cnv_w = coor_new->w[i];				
			...
			gt1x_touch_down(cnv_x, cnv_y, cnv_w, i);
			//GTP_DEBUG("(%d)(%d, %d)[%d]", i, cnv_x, cnv_y, cnv_w);
		} else if (coor_old->x[i] != -1) {
			touch_num--;
#if GTP_ICS_SLOT_REPORT
			gt1x_touch_up(i);
#endif
	...
	if (touch_num) {	/* restart timer */
		ktime_t ktime_now = hrtimer_cb_get_time(timer);
		input_sync(rate_boost->input_dev);
		hrtimer_forward(timer, ktime_now, ktime_set(0, RB_REPORT_RATE));
...
#endif
		input_sync(rate_boost->input_dev);
		rate_boost->record_empty = 1;
		//GTP_DEBUG("Rate boost timer stopped.");
		return HRTIMER_NORESTART;
	}	
}

/** 
 * rate_boost_init - initialization for rate boost module 
 * @dev: pointer to the input device
 */
int rate_boost_init(struct input_dev *dev)
{	
	rate_boost.input_dev = dev;
	spin_lock_init(&rate_boost.lock);
	hrtimer_init(&rate_boost.xtimer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);   /* attributes needs change*/
	
    rate_boost.xtimer.function = rate_boost_func;
	rate_boost.record_empty = 1;
	coor_new = &rate_boost.buffer0;
	coor_old = &rate_boost.buffer1;
	memset(coor_new, -1, sizeof(struct coordinate));
	memset(coor_old, -1, sizeof(struct coordinate));
	
	return 0;
}
#endif /* End GTP_RATE_BOOST */
