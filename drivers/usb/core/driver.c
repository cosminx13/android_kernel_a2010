...
void usb_hnp_polling_work(struct work_struct *work)
{
	...
	u8 *status = NULL;
	dev_err(&udev->dev, "BEGIN usb_hnp_polling_work\n");
	...
	if (ret < 0) {
		...
		dev_err(&udev->dev, "HNP polling failed. status %d\n", ret);
		goto out;
	}

	if (!(*status & (1 << HOST_REQUEST_FLAG))) {
        dev_err(&udev->dev, "usb_hnp_polling_work - goto reschedule!!\n");
		goto reschedule;
    }
...
#if 1
	dev_err(&udev->dev, "usb_hnp_polling_work - start_hnp\n");
	unbind_no_pm_drivers_interfaces(udev);
	udev->do_remote_wakeup = device_may_wakeup(&udev->dev);
	dev_err(&udev->dev, "usb_hnp_polling_work - before usb_suspend_both\n");
	ret = usb_suspend_both(udev, PMSG_USER_SUSPEND);
	dev_err(&udev->dev, "usb_hnp_polling_work - after usb_suspend_both\n");
#endif
    
stop_hnp:
#if 0
	//msleee(5000);
	dev_err(&udev->dev, "usb_hnp_polling_work - STOP_RECORD_OPSTATE\n");
	spin_lock(&mtk_musb->lock);
	musb_hnp_stop(mtk_musb);
	spin_unlock(&mtk_musb->lock);
	musb_set_host_request_flag(mtk_musb, 0);
	dev_err(&udev->dev, "usb_hnp_polling_work - MUSB_HNP_STOP\n");
#endif
	if (ret)
		dev_err(&udev->dev, "suspend failed\n");
	goto out;

reschedule:
	dev_err(&udev->dev, "usb_hnp_polling_work - schedule_delayed_work\n");
